...
class EngineArgs:
    ...
    # AgilePruner token-pruning flags (Phase 2.1)
    ...
    def __post_init__(self):
        ...
        # AgilePruner validation (Phase 2.1)
        if self.agilepruner_enable:
            if not (0.0 < self.agilepruner_ratio <= 1.0):
                raise ValueError(
                    f"--agilepruner-ratio must be in (0, 1], got {self.agilepruner_ratio}"
                )
            if self.agilepruner_tau_max <= 0.0:
                raise ValueError(
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # AgilePruner arguments (Phase 2.1)
        agilepruner_group = parser.add_argument_group(
            title="AgilePruner",
            description="Efficient visual token pruning for VL models via AgilePruner.",
        )
        agilepruner_group.add_argument(
            "--agilepruner-enable",
            dest="agilepruner_enable",
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Enable AgilePruner visual-token pruning for VL models.",
        )
        agilepruner_group.add_argument(
            "--agilepruner-ratio",
            dest="agilepruner_ratio",
            type=float,
            default=0.5,
            help="Fraction of visual tokens to KEEP (0, 1]. Default: 0.5.",
        )
        agilepruner_group.add_argument(
            "--agilepruner-tau-max",
            dest="agilepruner_tau_max",
            type=float,
            default=0.25,
            help="Maximum temperature tau for surrogate CLS score. Must be > 0. Default: 0.25.",
        )
        agilepruner_group.add_argument(
            "--agilepruner-erank-avg",
            dest="agilepruner_erank_avg",
            type=float,
            default=95.0,
            help="Target average effective rank for layer-wise attention matrices. Default: 95.0.",
        )
        return parser
    ...
    def create_model_config(self) -> ModelConfig:
        ...
        return ModelConfig(
            ...
            agilepruner_enable=self.agilepruner_enable,
            agilepruner_ratio=self.agilepruner_ratio,
            agilepruner_tau_max=self.agilepruner_tau_max,
            agilepruner_erank_avg=self.agilepruner_erank_avg,
        )
