...
# ---------------------------------------------------------------------------
# AgilePruner — helper (Phase 3.2)
# ---------------------------------------------------------------------------

def _get_pruned_count(n_tokens: int, agilepruner_enable: bool,
                      agilepruner_ratio: float) -> int:
    """Apply AgilePruner ratio to a visual-token count.

    Single source of truth used by BOTH stage-1 (placeholder emission in
    the input processor) and stage-2 (CudaGraph buffer allocation), so
    they always agree on K = round(ratio * N).
    """
    if not agilepruner_enable or agilepruner_ratio >= 1.0:
        return n_tokens
    return max(round(agilepruner_ratio * n_tokens), 4)
...
class Qwen3VLMultiModalProcessor(BaseMultiModalProcessor[Qwen3VLProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        def get_image_replacement_qwen3vl(item_idx: int):
            ...
            # AgilePruner (Phase 3.2): compress placeholder count if enabled.
            _mc = self.info.ctx.model_config
            num_tokens = _get_pruned_count(
                num_tokens,
                getattr(_mc, "agilepruner_enable", False),
                getattr(_mc, "agilepruner_ratio", 0.5),
            )
...
class Qwen3VLForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "model"):
        ...
        # AgilePruner (Phase 3.2): DP guard — warn when both are active.
        # The DP shard path in _process_image_input returns early via
        # run_dp_sharded_mrope_vision_model and bypasses the Phase 4.2
        # selection insertion point; pruning will not take effect.
        if getattr(vllm_config.model_config, "agilepruner_enable",
                   False) and self.use_data_parallel:
            logger.warning(
                "AgilePruner is enabled but use_data_parallel=True; the DP "
                "shard path bypasses the selection insertion point in "
                "_process_image_input. Visual-token pruning may not take "
    ...
    def get_encoder_cudagraph_per_item_output_tokens(
        ...
        # AgilePruner (Phase 3.2): apply the same compression as stage-1 so
        # CudaGraph replay-buffer shapes match the pruned placeholder count.
        _mc = self.model_config
        _ap_enable = getattr(_mc, "agilepruner_enable", False)
        _ap_ratio = getattr(_mc, "agilepruner_ratio", 0.5)
        return [
            _get_pruned_count(t * (h // m) * (w // m), _ap_enable, _ap_ratio)
            for t, h, w in grid_thw
        ]
